  ...
  const renderBoard = () => {
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < cols; col++) {
        ...
        const championAtPosition = positionedChampions.find(champ => champ.position && champ.position.row === row && champ.position.col === col);
        rowCells.push(<div key={`${row}-${col}`} onClick={() => handleCellClick(row, col)} onDragOver={handleDragOver} onDrop={e => handleDrop(e, row, col)} className="hex-cell my-[15px] mx-[39px]">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="champion-icon-wrapper">
                          <ChampionIcon name={championAtPosition.name} cost={championAtPosition.cost} size="md" /* Changed from 'sm' to 'md' for larger icons */ isCarry={championAtPosition.isCarry} onClick={() => !readonly && handleChampionClick(championAtPosition)} />
                        </div>
                      </TooltipTrigger>
                      <TooltipContent>
