      ...
      <div className="flex flex-wrap gap-2 mb-4">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                ...
                size="sm" 
                onClick={() => toggleSort(sortBy === 'name-asc' ? 'name-desc' : 'name-asc')}
              >
                {sortBy === 'name-desc' ? (
                  <><ArrowDownZA className="h-4 w-4 mr-1" /> Z-A</>
                ) : (
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                variant={sortBy.startsWith('difficulty') ? "default" : "outline"} 
                size="sm" 
                onClick={() => toggleSort(sortBy === 'difficulty-easy' ? 'difficulty-hard' : 'difficulty-easy')}
              >
                {sortBy === 'difficulty-hard' ? (
                  <><ArrowDown className="h-4 w-4 mr-1" /> Difficulty (Hard→Easy)</>
                ) : (
                  <><ArrowUp className="h-4 w-4 mr-1" /> Difficulty (Easy→Hard)</>
                )}
              </Button>
            </TooltipTrigger>
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                variant={sortBy.includes('liked') ? "default" : "outline"} 
                size="sm" 
                onClick={() => toggleSort(sortBy === 'most-liked' ? 'least-liked' : 'most-liked')}
              >
                {sortBy === 'least-liked' ? (
                  <><ThumbsDown className="h-4 w-4 mr-1" /> Least Liked</>
                ) : (
                  <><ThumbsUp className="h-4 w-4 mr-1" /> Most Liked</>
                )}
              </Button>
            </TooltipTrigger>
