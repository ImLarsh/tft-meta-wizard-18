...
import { Plus, Trash, X, Loader2, Crown, Shield } from 'lucide-react';
...
const SimpleCompForm: React.FC<SimpleCompFormProps> = ({ initialData, onSubmit, isSubmitting = false }) => {
  ...
  const handleAddItem = () => {
    if (!itemSearch || keyItems.includes(itemSearch)) return;
    setKeyItems([...keyItems, itemSearch]);
    setItemSearch("");
  };
  ...
  const handleAddChampionItem = () => {
    if (!itemSearch || newChampItems.includes(itemSearch)) return;
    setNewChampItems([...newChampItems, itemSearch]);
    setItemSearch("");
  };
  ...
  const getFilteredChampions = () => {
    return filteredChampions.filter(champ => 
      champ.toLowerCase().includes(championsSearch.toLowerCase())
    );
  };

  const getFilteredTraits = () => {
    return availableTraits.filter(trait => 
      trait.toLowerCase().includes(traitsSearch.toLowerCase())
    );
  };
            ...
            <Card className="p-6 border border-primary/20 shadow-sm">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Popover>
                      ...
                      <PopoverContent className="w-full p-0" align="start">
                        <ScrollArea className="h-[200px]">
                          {getFilteredItems().map((name, index) => (
                            <Button
                              key={index}
                              variant="ghost"
                              className="w-full justify-start rounded-none text-left font-normal"
                              onClick={() => setItemSearch(name)}
                            >
                              {name}
                            </Button>
                          ))}
                    ...
                    <Button 
                      ...
                      disabled={!itemSearch || newChampItems.includes(itemSearch)}
                      size="sm"
                    >
              ...
              {filteredChampions.length > 0 && (
                <div className="mt-4 bg-secondary/20 p-4 rounded-md">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {getFilteredChampions().slice(0, 10).map((champ, index) => (
                      <Badge 
                        key={index}
                        variant="outline"
                        className="cursor-pointer hover:bg-secondary"
                        onClick={() => {
                          setNewChampName(champ);
                          setChampionsSearch("");
                        }}
                      >
                        {champ}
                    ...
                    {getFilteredChampions().length > 10 && (
                      <Badge variant="outline">+{getFilteredChampions().length - 10} more</Badge>
                    )}
                  </div>
                </div>
              )}
          ...
          <TabsContent value="traits" className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <div className="md:col-span-2">
                  <Popover>
                    ...
                    <PopoverContent className="w-full p-0" align="start">
                      <ScrollArea className="h-[200px]">
                        {getFilteredTraits().map((name, index) => (
                          <Button
                            key={index}
                            variant="ghost"
                            className="w-full justify-start rounded-none text-left font-normal"
                            onClick={() => {
                              setNewTraitName(name);
                              setTraitsSearch("");
                            }}
                          >
                            {name}
              ...
              <div className="bg-secondary/20 p-4 rounded-md">
                <h4 className="text-sm font-medium mb-2">Available Traits</h4>
                <div className="flex flex-wrap gap-2">
                  {getFilteredTraits().slice(0, 20).map((trait) => (
                    <Badge 
                      key={trait} 
                      variant="outline"
                      className="cursor-pointer hover:bg-secondary"
                      onClick={() => {
                        setNewTraitName(trait);
                        setTraitsSearch("");
                      }}
                    >
                      {trait}
                  ...
                  {getFilteredTraits().length > 20 && (
                    <Badge variant="outline">+{getFilteredTraits().length - 20} more</Badge>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  disabled={!itemSearch || keyItems.includes(itemSearch)}
                >
                  Add Item
                </Button>
              </div>
              ...
              <div className="bg-secondary/20 p-4 rounded-md">
                <h4 className="text-sm font-medium mb-2">Common Items</h4>
                <div className="flex flex-wrap gap-2">
                  {commonItems.slice(0, 15).map((item, idx) => (
                    <Badge 
                      key={idx} 
                      variant="outline"
                      className="cursor-pointer hover:bg-secondary"
                      onClick={() => setItemSearch(item)}
                    >
                      {item}
                    </Badge>
                  ))}
                  <Badge variant="outline">+{commonItems.length - 15} more</Badge>
                </div>
              </div>
            </div>
          </TabsContent>
