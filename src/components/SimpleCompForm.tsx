...
const championFormSchema = z.object({
  name: z.string().min(1, {
    message: "Please select a champion"
  }),
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="general" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <FormField control={form.control} name="tier" render={({
              field
            }) => <FormItem>
              ...
              <FormField control={form.control} name="playstyle" render={({
              field
            }) => <FormItem>
              ...
              <FormField control={form.control} name="tftVersion" render={({
              field
            }) => <FormItem>
                    <FormLabel>TFT Version</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value || availableSets[0] || "Set 10"}>
                      <FormControl>
                        <SelectTrigger>
                      ...
                      <SelectContent>
                        {availableSets.map(setName => <SelectItem key={setName} value={setName}>
            ...
            field
                  ...
                  <FormControl>
                    <Textarea placeholder="This comp focuses on..." className="min-h-[100px]" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />

            <FormField control={form.control} name="patch" render={({
            field
          }) => <FormItem>
          ...
          <TabsContent value="champions" className="space-y-6">
            <Card className="p-6 border border-primary/20 shadow-sm">
              ...
              {finalComp.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                  {finalComp.map((champ, index) => <div key={index} className={`flex p-3 rounded-md group transition-all border ${champ.isCarry ? 'bg-primary/10 border-primary' : 'bg-card border-border hover:border-primary/40'}`}>
                      ...
                      <div className="ml-3 flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => removeChampion(index)}>
                        ...
                        <div className="flex items-center gap-1 text-xs mt-1">
                          <span className={`px-1.5 py-0.5 rounded ${champ.cost >= 4 ? 'bg-primary/20 text-primary' : 'bg-muted'}`}>
                            {champ.cost} Cost
                          </span>
                          
                          {champ.isCarry && <span className="bg-primary/20 text-primary px-1.5 py-0.5 rounded flex items-center">
                              <Crown className="h-3 w-3 mr-0.5" />
                              Carry
                            </span>}
                        </div>
                        
                        {champ.position && <div className="mt-2 text-xs text-muted-foreground">
                            <MapPin className="h-3 w-3 inline mr-1" />
                            Position: ({champ.position.row}, {champ.position.col})
                          </div>}

                        {champ.items && champ.items.length > 0 && <div className="mt-2">
                            <p className="text-xs text-muted-foreground mb-1">Items:</p>
                            <div className="flex flex-wrap gap-1">
                              {champ.items.map((item, idx) => <span key={idx} className="text-xs bg-secondary/30 px-1.5 py-0.5 rounded">
                                  {item}
                                </span>)}
                            </div>
                          </div>}
                ...
                </div> : <div className="text-center py-6 bg-muted/40 rounded-md mb-6">
              ...
              <div className="space-y-4 border border-border rounded-md p-4 bg-card/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={newChampName} onChange={e => setNewChampName(e.target.value)} className="w-full p-2 rounded-md border border-input bg-background">
                      <option value="">Select a champion</option>
                      {Object.keys(currentTraitMap).map(name => <option key={name} value={name}>{name}</option>)}
                    </select>
                  </div>
                  
                  <div className="flex items-end gap-4">
                    <div className="flex items-center gap-2">
                      <input type="checkbox" id="carry-checkbox" checked={newChampIsCarry} onChange={e => setNewChampIsCarry(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                    ...
                    <div className="flex-1">
                      <Select value={selectedItem} onValueChange={setSelectedItem}>
                        ...
                        <SelectContent>
                          {commonItems.map(item => <SelectItem key={item} value={item}>
                              {item}
                            </SelectItem>)}
                        </SelectContent>
                      </Select>
                    ...
                    <Button type="button" size="sm" onClick={addItemToChampion} disabled={!selectedItem || selectedItems.length >= 3}>
                ...
                {selectedItems.length > 0 && <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedItems.map((item, index) => <div key={index} className="flex items-center gap-1 bg-secondary/30 rounded-md px-2 py-1">
                          <span className="text-xs">{item}</span>
                          <Button type="button" variant="ghost" size="sm" className="h-4 w-4 p-0 ml-1" onClick={() => removeItemFromSelection(index)}>
                            <X className="h-3 w-3" />
                          </Button>
                        </div>)}
                    </div>
                ...
                <Button type="button" onClick={handleAddChampion} disabled={!newChampName} className="w-full">
                  Add Champion
                </Button>
              </div>
          ...
          <TabsContent value="positioning" className="space-y-6">
            <div className="bg-card/50 border border-border rounded-lg p-4">
              ...
              <BoardPositioning champions={finalComp} onUpdatePositions={handleUpdatePositions} readonly={false} />
            </div>
          </TabsContent>
        </Tabs>

        <div className="flex justify-end pt-6 border-t border-border">
          <Button type="submit" disabled={isSubmitting || finalComp.length === 0} className="w-full md:w-auto">
